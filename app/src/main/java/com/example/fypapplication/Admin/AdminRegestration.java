...
public class
AdminRegestration extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        top = findViewById(R.id.namecompany2);
        ...
        mAuth.createUserWithEmailAndPassword(mailg, passp).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if (task.isSuccessful()) {
                    Admin_Addtofirebase tofirebase = new Admin_Addtofirebase(mailg, passp, emp);
                    FirebaseDatabase.getInstance().getReference("admins").child(emp).setValue(tofirebase).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
